...
class DDPG(RLAlgorithm, Serializable):
    ...
    def __init__(
        ...
        super(DDPG, self).__init__(**base_kwargs)
    ...
    def _init_critic_update(self):
        ...
        self._qf1_params = self._qf1.get_params_internal()
        self._qf2_t = self._qf2.get_output_for(
            self._obs_pl, self._action_pl, reuse=True)
        self._qf2_params = self._qf2.get_params_internal()
        
        policy_act = self._policy.get_distribution_for(
                        self._obs_next_pl, reuse=True).determ_action

        with tf.variable_scope('target'):
            qf1_next_target_t = self._qf1.get_output_for(self._obs_next_pl, policy_act)  # N
            self._qf1_target_params = self._qf1.get_params_internal()
            qf2_next_target_t = self._qf2.get_output_for(self._obs_next_pl, policy_act)  # N
            self._qf2_target_params = self._qf2.get_params_internal()
            min_next_target_t = tf.minimum(qf1_next_target_t, qf2_next_target_t)

        ys = tf.stop_gradient(
            self._scale_reward * self._reward_pl +
            (1 - self._terminal_pl) * self._discount * min_next_target_t 
        )  # N
    ...
    def _init_actor_update(self):
        ...
        policy_loss_t = tf.reduce_mean(-min_log_target_t) # also try not taking the min here
        ...
        policy_train_op = tf.train.AdamOptimizer(self._policy_lr).minimize(
            loss=policy_loss_t + policy_dist.reg_loss_t,
    ...
    def _init_target_ops(self):
        ...
        source1_params = self._qf1_params
        target1_params = self._qf1_target_params
        source2_params = self._qf2_params
        target2_params = self._qf2_target_params

        self._target_ops = [
            tf.assign(target, (1 - self._tau) * target + self._tau * source)
            for target, source in zip(target1_params, source1_params)
        ] + [
            tf.assign(target, (1 - self._tau) * target + self._tau * source)
            for target, source in zip(target2_params, source2_params)
        ]


    @overrides
    def _init_training(self, env, policy, pool):
        super(DDPG, self)._init_training(env, policy, pool)
    ...
    def log_diagnostics(self, batch):
        ...
        feed_dict = self._get_feed_dict(batch)
        qf1, qf2, td_loss1, td_loss2 = self._sess.run(
            [self._qf1_t, self._qf2_t,self._td_loss1_t, self._td_loss2_t], feed_dict)
